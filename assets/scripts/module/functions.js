...
export function filtrarPorCategoria(array, categoria) {
  const productosPorCategoria = array.filter(
...
export function agregarTarjetas(array, elementoDestino, productosSelec) {
  if (array.length === 0) {
    return (elementoDestino.innerHTML = `
...
export function crearTarjeta(lista, productosSelec) {
  let color = productosSelec.some((producto) => producto._id == lista._id)
    ? "bg-secondary"
    : "bg-white";
  if (lista.disponibles === 0) {
    return `
        <div class="card text-center col-md-3 m-4 cards-jf" style="width: 18rem;">
        <div class="card-body">
            <h5 class="card-title pt-1">${lista.producto}</h5>   
            <img src="${lista.imagen}" height="200rem" class="card-img-top card-img" id="imageCard" alt="${lista.producto}">

            <div class="precioStock">
            ...
            <div class="contenedorBotones">
                ...
                <button type="button" class="btn colorBotones" data-bs-toggle="modal" data-bs-target="#${lista.modal}">
                Detalles
                </button>
            </div>
            ...
            <div class="modal-dialog">
                <div class="modal-content modal-jf">
                <div class="modal-header">
                    <h1 class="modal-title fs-5 modal-titulo" id="exampleModalLabel">${lista.producto}</h1>
    ...
    return `
        ...
        <div class="card-body">
            ...
            <div class="modal-dialog">
                <div class="modal-content modal-jf">
                <div class="modal-header modal-titulo-div">
                    <h1 class="modal-title fs-5 modal-titulo" id="exampleModalLabel">${lista.producto}</h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
...
export function pintarCarrito(lista, contenedor) {
  let cantidad = 1;
  lista.forEach((producto) => {
    contenedor.innerHTML += `
        <tr class="text-center">
            <td class="shopProductoImg"><div ><img src="${producto.imagen}" alt="Imagen producto" class="imgShop"></div>${producto.producto}</td>
            <td>${cantidad}</td>
